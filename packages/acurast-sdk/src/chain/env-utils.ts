...
export function getProcessorEncryptionKey(
  ...
  const key =
    ...
    findProcessorEncrypionKey(assignment, ['SECP256k1Encryption', 'secp256k1']) ??
    /* backwards compatibility */ findProcessorEncrypionKey(assignment, ['SECP256r1', 'p256'])

  return key !== undefined ? { ...key, publicKey: key.publicKey.replace('0x', '') } : undefined
}
